...
export function DashboardPage(){
 ...
 const load=useCallback(async()=>{if(!business)return;const [{data:a},{count:c},{data:t},{count:w},{data:stock}]=await Promise.all([
  ...
  supabase.from('inventory_products').select('stock_quantity,min_stock,active').eq('business_id',business.id).eq('active',true)
 ]);setRows((a||[]) as Row[]);setClientCount(c||0);setExpenses((t||[]).reduce((s:any,x:any)=>s+Number(x.amount||0),0));setWaitlistCount(w||0);setLowStockCount((stock||[]).filter((x:any)=>Number(x.stock_quantity)<=Number(x.min_stock)).length)},[business])
 useEffect(()=>{load()},[load])
 useEffect(()=>{if(!business)return;const ch=supabase.channel(`dashboard:${business.id}`).on('postgres_changes',{event:'*',schema:'public',table:'appointments',filter:`business_id=eq.${business.id}`},load).on('postgres_changes',{event:'*',schema:'public',table:'financial_transactions',filter:`business_id=eq.${business.id}`},load).on('postgres_changes',{event:'*',schema:'public',table:'clients',filter:`business_id=eq.${business.id}`},load).on('postgres_changes',{event:'*',schema:'public',table:'waitlist_entries',filter:`business_id=eq.${business.id}`},load).on('postgres_changes',{event:'*',schema:'public',table:'inventory_products',filter:`business_id=eq.${business.id}`},load).subscribe(s=>setLive(s==='SUBSCRIBED'));return()=>{supabase.removeChannel(ch)}},[business,load])
 ...
 return <main className="admin-page"><AdminSidebar businessName={business.name} current="dashboard"/><section className="admin-content advanced-page">
  ...
  <div className="advanced-grid-2"><section className="panel-card action-card"><div><Clock3/><div><h2>Lista de espera</h2><p>{waitlistCount} cliente(s) aguardando oportunidade de horário.</p></div></div><button className="button button-secondary" onClick={()=>navigate('/painel/lista-espera')}>Gerenciar fila</button></section><section className={`panel-card action-card ${lowStockCount?'attention-card':''}`}><div><Scissors/><div><h2>Estoque operacional</h2><p>{lowStockCount?`${lowStockCount} item(ns) no estoque mínimo.`:'Nenhum alerta de estoque baixo.'}</p></div></div><button className="button button-secondary" onClick={()=>navigate('/painel/estoque')}>Abrir estoque</button></section></div>
